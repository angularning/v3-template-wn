  ...
  populationInfoDetail({ id })?.then((item) => {
    if (item.code === 0) {
      ...
      if (configStore.appConfig.entry) {
        ...
        state.ruleForm.mobile = encryptionNumber(
          configStore.appConfig.entry,
      ...
      labelsInfoList(params)?.then((res) => {
        if (res.code === 0) {
          ...
          const labels = state.labelsList;
          const result =
            item.body.labels &&
            item.body.labels.split(",").map((r) => {
              return Number(r);
            });
          state.ruleForm.labels = result;
        }
      });
    }
  });
...
const submitForm = () => {
  ...
  formRef.value.validate((valid: boolean) => {
    if (valid) {
      if (props.type === "add") {
        populationInfoAdd({ ...params })?.then((item) => {
          if (item.code === 0) {
        ...
        populationInfoEdit({ ...params })?.then((item) => {
          if (item.code === 0) {
